...
class SpCategorieController extends Controller
{
    /**
    ...
    public function storeNewCategorie(Request $request)
    {
        ...
        // Association à l'utilisateur connecté
        auth()->user()->categories()->attach($categorie->id);

        // Retourner une réponse JSON pour Inertia
        return redirect()->route('mds-categories.newCreate')
    ...
    public function show($slug)
    {
        ...
        return Inertia::render('Managers/Supermarche/CategorieProduits', [
            'categorie' => $categorie,
            'produits' => $produits,
            'appUrl' => config('app.url'),
        ]);
    ...
    public function update(Request $request, string $id)
    {
        ...
        if (!$categorie) {
            return redirect()->route('mds-categories.index')->with('error', 'Catégorie non trouvée');
        }
        ...
        try {
            ...
            return redirect()->route('mds-categories.newCreate')->with('success', 'Catégorie mise à jour avec succès');
