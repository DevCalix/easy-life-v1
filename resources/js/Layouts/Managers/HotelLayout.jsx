...
import "../../../css/hotelReservation/HotelLayout.css";

export default function HotelsLayout({ children, title }) {
            ...
            <div className={`sidebar bg-dark text-white ${sidebarCollapsed ? 'collapsed' : ''}`}>
                ...
                <ul className="nav flex-column px-2 pb-4 mt-3">
                    <li className="nav-item">
                        <Link
                            href={route('htm-reservations.index')}
                            className={`nav-link text-white d-flex align-items-center py-3 rounded-3 ${
                                isActive('htm-reservations.*') ? activeClass : ''
                            }`}
                        >
                    ...
                    <li className="nav-item mt-1">
                        ...
                        <Link
                            href={route('mgs-hotels.index')}
                            className={`nav-link text-white d-flex align-items-center py-3 rounded-3 ${
                                isActive('mgs-hotels.*') ? activeClass : ''
                            }`}
                        >
                            <i className="bi bi-box-seam fs-5"></i>
                            {!sidebarCollapsed && <span className="ms-3">Hotels</span>}
                        </Link>

                        <Link
                            href={route('mgs-chambres.index')}
                            className={`nav-link text-white d-flex align-items-center py-3 rounded-3 ${
                                isActive('mgs-chambres.*') ? activeClass : ''
                            }`}
                        >
                            <i className="bi bi-tags fs-5"></i>
                            {!sidebarCollapsed && <span className="ms-3">Chambre</span>}
                        </Link>

                        <Link
                            href={route('mgs-promotions.index')}
                            className={`nav-link text-white d-flex align-items-center py-3 rounded-3 ${
                                isActive('mgs-promotions.*') ? activeClass : ''
                            }`}
                        >
                            <i className="bi bi-shop fs-5"></i>
                            {!sidebarCollapsed && <span className="ms-3">Promotion</span>}
                        </Link>
                    </li>
