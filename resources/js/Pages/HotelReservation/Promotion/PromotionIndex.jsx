...
const PromotionIndex = () => {
    ...
    return (
        <AdminLayout title="Liste des Promotions">
            <div className="container py-5">
                <div className="d-flex justify-content-between align-items-center mb-4">
                    <h1 className="mb-4 montserrat-normal fw-bold text-primary">Liste des Promotions</h1>
                    <Link href={route('promotions.create')} className="btn btn-primary montserrat-normal">
                        Faire une promotion
                    </Link>
                </div>
                ...
                <div className="mb-4">
                    <input
                        ...
                        onChange={(e) => {
                            window.location.href = `/reservation-hotel/promotions?search=${e.target.value}`;
                        }}
                        ...
                        <table className="table table-striped montserrat-normal">
                            <thead>
                                <tr className="montserrat-normal">
                                    <th>
                                        <Link
                                            href={`/reservation-hotel/promotions?sort_by=date_debut&sort_order=${filters.sort_order === 'asc' ? 'desc' : 'asc'}`}
                                        >
                                            Date de Début
                                        </Link>
                                    </th>
                                    <th>
                                        <Link
                                            href={`/reservation-hotel/promotions?sort_by=date_fin&sort_order=${filters.sort_order === 'asc' ? 'desc' : 'asc'}`}
                                        >
                                            Date de Fin
                                        </Link>
                                    </th>
                            ...
                            <tbody>
                                {promotions.data.map((promotion) => (
                                    <tr key={promotion.id}>
                                        ...
                                        <td>
                                            <Link
                                                href={`/reservation-hotel/promotions/${promotion.id}/edit`}
                                            ...
                                            <button
                                                onClick={() => {
                                                    if (confirm('Êtes-vous sûr de vouloir supprimer cette promotion ?')) {
                                                        router.delete(`/reservation-hotel/promotions/${promotion.id}`);
                                                    }
                                                }}
