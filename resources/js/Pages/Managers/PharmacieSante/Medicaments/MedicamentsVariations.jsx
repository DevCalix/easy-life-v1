...
const MedicamentVariationForm = ({ medicament, flash }) => {
    ...
    const handleDeleteVariation = async (variationId) => {
        if (confirm("Voulez-vous vraiment supprimer cette variation ?")) {
            await router.delete(route("sts-medicaments.variations.destroy", variationId), {
                preserveScroll: true,
            });
    ...
    const handleSubmit = (e) => {
        ...
        router.post(route("sts-medicaments.variations.store", medicament.id), formData, {
            forceFormData: true,
        });
    };

    return (
        <PharmaLayout title="Variations liées au médicament">
            
            <div className="container-fluid">
            <div className="d-flex justify-content-between align-items-center mb-4">
                <h3 className="fw-bold mb-4">Gérer les variations du médicament</h3>
                <Link href={route('sts-medicaments.index')} className="btn btn-warning montserrat-normal">
                    Retour
                </Link>
                </div>
