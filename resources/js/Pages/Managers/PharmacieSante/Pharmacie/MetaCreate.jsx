...
const MetaCreate = ({ pharmacie }) => {
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        post(route('pharmacies.informations-supplementaires.pharmacie', pharmacie.id), {
            onSuccess: () => {
    ...
    return (
        <PharmaLayout title="Gérer les informations supplémentaires">
            ...
            <div className="container-fluid">
                ...
                {pharmacie.metas.length > 0 ? (
                    <div className="card shadow p-4">
                        <ul className="list-group">
                            {pharmacie.metas.map((meta, index) => (
                                <li key={index} className="list-group-item d-flex justify-content-between align-items-center">
                                    <div>
                                        <strong>{meta.cle}:</strong> {meta.valeur}
                                    </div>
                                    <Link
                                        href={route('pharmacies.informations-supplementaires.destroy', [pharmacie.id, meta.id])}
                                        method="delete"
                                        as="button"
                    ...
                    <div className="alert alert-info">
                        Aucune information supplémentaire n'a été ajoutée pour ce pharmacie.
                    </div>
                )}
            </div>
        </PharmaLayout>
