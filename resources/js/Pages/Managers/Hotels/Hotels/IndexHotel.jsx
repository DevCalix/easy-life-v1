...
const IndexHotel = () => {
    ...
    const handleDelete = (id) => {
        if (confirm("Voulez-vous vraiment supprimer cet hôtel ?")) {
            Inertia.delete(route('mgs-hotels.destroy', id), { preserveScroll: true });
        }
    };
    ...
    return (
        <HotelsLayout title={"Liste des Hôtels"}>
            <div className="container py-2">
                <div className="d-flex justify-content-between align-items-center mb-4">
                    <h1 className="montserrat-normal fw-bold">Liste des Hôtels</h1>
                    <Link href={route('mgs-hotels.create')} className="btn btn-primary montserrat-normal">
                        Ajouter un Hôtel
                    </Link>
                </div>
                                    ...
                                    <tr key={hotel.id}>
                                        ...
                                        <td>
                                            <div className="d-flex gap-2">
                                                {/* Bouton Éditer */}
                                                <Link
                                                    href={route('mgs-hotels.edit', hotel.id)}
                                                ...
                                                {/* Gérer les images */}
                                                <Link
                                                    href={`/hotel/managers/hotels/${hotel.id}/images`}
                                                    className="btn btn-secondary btn-sm"
                                                >
                                                    Images
                                                </Link>
                                                <Link
                                                    href={`/hotel/managers/mgs-hotels/${hotel.id}/informations-supplementaires/create`}
