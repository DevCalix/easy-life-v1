...
const MetaCreate = ({ restaurant }) => {
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        post(route('mgs-restaurants.informations-supplementaires.restaurant', restaurant.random_id), {
            onSuccess: () => {
    ...
    return (
        <RestauLayout title="Informations supplémentaires">
            <div className="container">
                <div className="d-flex justify-content-between align-items-center mb-4">
                    <h1 className="montserrat-normal fw-bold">Informations supplémentaires</h1>
                    <Link href={route('mgs-restaurant.index')} className="btn btn-primary montserrat-normal">
                        Retour
                    </Link>
                </div>
                ...
                {restaurant.metas.length > 0 ? (
                    <div className="card shadow p-4">
                        <ul className="list-group">
                            {restaurant.metas.map((meta, index) => (
                                <li key={index} className="list-group-item d-flex justify-content-between align-items-center">
                                    <div>
                                        <strong>{meta.cle}:</strong> {meta.valeur}
                                    </div>
                                    <Link
                                        href={route('mgs-restaurants.informations-supplementaires.destroy', [restaurant.random_id, meta.id])}
                                        method="delete"
                                        as="button"
